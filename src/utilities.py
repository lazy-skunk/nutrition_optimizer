...
class Utilities:
    ...
    def parse_request_data(request: Request) -> dict:
        try:
            ...
            food_information_list = Utilities.convert_to_food_information(
                food_information_request_data
            )

            objective_request_data = request.json.get("objective")[0]
            objective = Utilities.convert_to_objective(objective_request_data)

            constraints_request_data = request.json.get("constraints")
            constraint_list = Utilities.convert_to_constraints(
    ...
    @staticmethod
    def convert_to_food_information(data: list) -> list:
        return [
            FoodInformation(
                **{
    ...
    @staticmethod
    def convert_to_objective(data: dict) -> Objective:
        return Objective(
            **{
                Utilities.camel_to_snake(key): value
    ...
    @staticmethod
    def convert_to_constraints(data: list) -> list:
        return [
            Constraint(
                **{
                    Utilities.camel_to_snake(key): value
